...
__global__ void concurrentKernel(Heap<float> *heap, float *items, int initSize, int testSize, int batchSize) {
...
__global__ void insertKernel(Heap<float> *heap, 
                             float *items, 
                             int arraySize, 
                             int batchSize) {
...
__global__ void deleteKernel(Heap<float> *heap, float *items, int arraySize, int batchSize) {
...
int main(int argc, char *argv[]) {
    ...
    float *oriItems = new float[initSize + arrayNum];
#ifdef ENABLE_SEQ 
    int *h_tItems = new int[arrayNum];
#endif
    for (int i = 0; i < initSize + arrayNum; ++i) {
        oriItems[i] = rand()/RAND_MAX;
#ifdef ENABLE_SEQ
    ...
    if (keyType == 1) {
        ...
        std::sort(oriItems, oriItems + initSize + arrayNum, std::greater<float>());
    }
    ...
    // bitonic heap sort
    Heap<float> h_heap(batchNum * 2, batchSize, FLT_MAX);

    float *heapItems;
    Heap<float> *d_heap;

    cudaMalloc((void **)&heapItems, sizeof(float) * (arrayNum + initSize));
    cudaMemcpy(heapItems, oriItems, sizeof(float) * (arrayNum + initSize), cudaMemcpyHostToDevice);
    cudaMalloc((void **)&d_heap, sizeof(Heap<float>));
    cudaMemcpy(d_heap, &h_heap, sizeof(Heap<float>), cudaMemcpyHostToDevice);

    int smemSize = batchSize * 3 * sizeof(float);
    smemSize += (blockSize + 1) * sizeof(float) + 2 * batchSize * sizeof(float);
